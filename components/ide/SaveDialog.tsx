...
export default function SaveDialog({ fileName, onSave, onDiscard, onCancel }: SaveDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md overflow-hidden rounded-xl border border-[#3b434b] bg-[#161b22] shadow-2xl shadow-black/70">
        <div className="flex gap-4 p-5">
          ...
          <div>
            ...
            <span className="mt-3 inline-flex items-center gap-1.5 rounded bg-[#21262d] px-2 py-1 text-[11px] text-[#aeb8c2]">
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-[#30363d] bg-[#11161d] px-5 py-3.5">
          <button onClick={onCancel} className="rounded-md px-3 py-2 text-sm font-medium text-[#c9d1d9] transition hover:bg-[#21262d]">
            Cancel
          </button>
          ...
          <button onClick={onSave} className="rounded-md bg-[#007acc] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#1685d1]">
            Save
          </button>
        </div>
      </div>
